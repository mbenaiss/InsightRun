...
- Régénération avec confirmation ; l’ancien plan est conservé si la tentative échoue. Le changement de début recalcule le plan pour garder la date de course et est indisponible lorsque des séances ont déjà été validées. « Commencer aujourd’hui » est aussi désactivé si la date de course ne peut pas être respectée dans une préparation de 4 à 24 semaines.
...
| Abonnement requis | Restriction vérifiée dans le modèle et dans l’interface ; ouverture du paywall avec les offres annuelle et mensuelle, sans lancer la génération ni effectuer d’achat |
...
- Backend : **119 tests réussis, 422 assertions**, vérification TypeScript, lint et build réussis. Appel réel de la préversion avec des dates calendaires du 16 septembre au 14 octobre : HTTP 200 en **38,5 secondes**, cinq semaines, course présente dans la cinquième semaine.
...
- Après les dernières corrections : **166 tests unitaires iOS réussis**, puis le nouveau test UI du formulaire réussi. Le dernier contrôle des dates de début passe les **23 tests ciblés** de stabilité des plans.
...
- Parcours UI contrôlés : `ui-recovery-final.xcresult` ; consentement : premier test réussi dans `ui-permissions-v5.xcresult` ; abonnement : `ui-subscription-v2.xcresult` ; dates de début : `start-date-final.xcresult`.
