...
export default function FeaturesPage() {
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-emerald-100 max-w-xl leading-relaxed">
              Manage system capabilities and roll out new features safely. Toggle functionality on or off instantly across all clients.
            </p>
          </div>
      ...
      {error && <div className="bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 flex items-center gap-3"><span className="text-xl">⚠️</span>{error}</div>}
      {success && <div className="bg-green-50 text-green-600 p-4 rounded-xl border border-green-100 flex items-center gap-3"><span className="text-xl">✓</span>{success}</div>}
        ...
        <div className="bg-white rounded-xl shadow-lg border border-emerald-100 p-6 relative overflow-hidden animate-in fade-in slide-in-from-top-4 duration-200">
          ...
          <form onSubmit={addFeature} className="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div className="md:col-span-5">
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">Feature Key</label>
            ...
            <div className="md:col-span-4 flex flex-col justify-center">
              <label className="block text-sm font-semibold text-gray-700 mb-3">Initial State</label>
              <div className="flex items-center gap-3">
                ...
                <span className={`text-sm font-medium px-2 py-1 rounded ${newFeatureEnabled ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>
        ...
        <div className="divide-y divide-gray-100">
          {Object.keys(features).length === 0 ? (
            <div className="p-12 text-center text-gray-500 flex flex-col items-center">
              <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mb-4 text-3xl opacity-50">⚙️</div>
            ...
            Object.entries(features).map(([key, value]) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-2 h-2 rounded-full ${value ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]' : 'bg-gray-300'}`}></div>
                  <div>
                    <h3 className="text-sm font-bold text-gray-900 font-mono tracking-tight">{key}</h3>
                ...
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-3">
                    <span className={`text-xs font-medium transition-colors ${value ? 'text-emerald-600' : 'text-gray-400'}`}>
                      {value ? 'ON' : 'OFF'}
                    </span>
                    <button
      ...
      <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-5 flex gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-blue-700 mt-1 leading-relaxed">
            Changes take effect immediately on the server. Client applications (iOS/Web) update their configuration within 5 minutes of their next foreground activity or restart. The system uses snake_case for keys to maintain compatibility.
          </p>
        </div>
      </div>
    </div>
