...
- **Analyses en cache** : version du contexte et référence cardiaque enregistrées avec chaque analyse. Une ancienne analyse ou une référence modifiée déclenche une nouvelle génération. En cas d’échec réseau, l’ancienne entrée reste conservée sans afficher son interprétation devenue incohérente. La migration de l’ancien schéma SwiftData est testée sur un fichier persistant.
...
| Migration | Ancienne base d’analyses sur disque migrée vers le nouveau modèle sans perte ; les anciennes analyses sont identifiées comme devant être recalculées. |
